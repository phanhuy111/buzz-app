...
import { useVector, withRubberBandClamp } from 'utils';

import type { TargetImageInfo } from './LightBox';
import type { AnimationParams } from './ProviderBox';
...
const maxScale = 6;
export const LightImageModal = ({ activeImage, onClose }: LightImageModalProps) => {
    const { layout, position, imageElement, onLongPress, tapToClose, onTap } = activeImage;
    ...
    const targetY = useSharedValue((dimensionHeight - targetHeight) / 2 - headerHeight);
    ...
    useEffect(() => {
        ...
        backdropOpacity.value = withTiming(1, timingConfig);
        const backhanderListener = BackHandler.addEventListener('hardwareBackPress', () => {
            closeModal();
            return true;
        });
        return () => {
    ...
    const pinchGesture = Gesture.Pinch()
        ...
        .onUpdate(({ scale: s, focalX, focalY, numberOfPointers }) => {
            ...
            const nextScale = withRubberBandClamp(s * scaleOffset.value, 0.55, maxScale, [
                1,
                maxScale,
            ]);
            ...
            translation.x.value =
                adjustedFocal.x.value + ((-1 * nextScale) / scaleOffset.value) * origin.x.value;
            translation.y.value =
                adjustedFocal.y.value + ((-1 * nextScale) / scaleOffset.value) * origin.y.value;
        })
        .onEnd(() => {
            'worklet';
            if (scale.value < 1) {
                ...
                const diffX = nextTransX + offset.x.value - (newWidth - layout.width) / 2;
                ...
                const diffY = nextTransY + offset.y.value - (newHeight - height.value) / 2;
